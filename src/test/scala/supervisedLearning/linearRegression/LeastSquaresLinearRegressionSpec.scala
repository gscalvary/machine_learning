...
class LeastSquaresLinearRegressionSpec extends FunSuite {
	...
	test("Data is not returned if there are less observations than features.") {
		...
		assert(linearLeastSquaresRegressor.getCovariance.isEmpty)
	}
	
	test("Data is not returned if the observations with offsets matrix is not full rank.") {
	...
	test("A covariance matrix of the correct size is computed given a valid training set.") {
		...
		assert(linearLeastSquaresRegressor.getCovariance.get.rows == 3)
		assert(linearLeastSquaresRegressor.getCovariance.get.cols == 3)
	}
